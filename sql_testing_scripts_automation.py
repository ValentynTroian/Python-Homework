...
def read_config():
    ...
    try:
        ...
        print(
            'Can\'t find [config.json] in current directory.'
            'Please ensure that the file is located there')
        exit(-1)

    except OSError:
        print(
            'OSError during [config.json] reading. Please ensure that your disk isn\'t full')
        exit(-2)
...
def connect_to_sql_server(config):
    ...
    try:
        ...
        print(
            'Can\'t connect to SQL Server for long time.'
            'Please check [config.json] parameters and try again')
        exit(-3)
    except InterfaceError:
        print(
            'Can\'t connect to SQL Server.'
            'Please check [DRIVER] attribute in [config.json] and try again')
        exit(-4)
    except OperationalError:
        print(
            'Can\'t connect to SQL Server.'
            'Please check [SQL_SERVER_NAME] attribute in [config.json] and try again')
        exit(-5)
    except ProgrammingError:
        print(
            'Can\'t connect to SQL Server.'
            'Please check [DB_NAME] attribute in [config.json] and try again')
        exit(-6)
...
# The function isn\'t called in main by default.
def find_sql_files_with_traverse(config):
    ...
    for _, _, files in walk(config['PATH_TO_SCRIPTS_FOLDER']):
        for file in files:
            if file.endswith('.sql'):
                files_list_tr.append(
                    path.join(
                        config['PATH_TO_SCRIPTS_FOLDER'],
                        file))
...
def exec_sql_and_save_to_csv(files_list, connection, config):
    ...
    try:
        with open(csv_file, 'w', newline='') as file:
            f.write(
                'FILE_NAME,EXECUTION_STATUS,ERROR_MESSAGE,'
                'EXECUTION_TIME,OBJECT_NAME,TEST_NAME,COUNT,RESULT\n')

    except OSError:
        print(
            'OSError during',
            csv_file,
            'creating. Please ensure that your disk isn\'t full')
        exit(-7)
    ...
    for file in files_list:
        ...
        # Errors handling during sql file opening
            ...
            print(
                'OSError during ',
                file,
                ' opening. Please ensure that your disk isn\'t full')
        ...
        try:
            ...
            # Getting results of query execution, adding sql files names,
            # execution status and time to csv output list
            file_output = [
                [file] +
                ['Query executed successfully'] +
                [''] +
                [exec_time] +
                list(tup) for tup in sql_output]
            print(file_output)

        # Writing error messages during query execution to csv file and ending
        # the current loop cycle
            ...
            with open(csv_file, 'a', newline='') as f:
                ...
                f.write(
                    str(e)[
                        str(e).index('[SQL Server]') +
                        len('[SQL Server]'):str(e).index('.')])
                f.write('\n')
                continue
        ...
        try:
            ...
            print('OSError during writing to ', csv_file,
                  '. Please ensure that your disk isn\'t full')
...
def send_email(csv_file, config):
    ...
    message['Subject'] = config['ENV'] + \
        ' Testing in: ' + config['PATH_TO_SCRIPTS_FOLDER']
...
def update_csv_in_gspread(csv_file, config):
    ...
    try:
        ...
        print(
            'Invalid grant: Not a valid email or user ID. Please check your ',
            config['CREDENTIALS_FILE_NAME'])
        exit(-10)
    ...
    try:
        ...
        print(
            'Can\'t find the spreadsheet. Please ensure that you created the spreadsheet '
            'with name specified in the ',
            config['CREDENTIALS_FILE_NAME'])
        exit(-11)
...
def main():
    ''' This main function runs the main program flow'''
    ...
    call_csv_file = exec_sql_and_save_to_csv(
        call_files_list, call_connection, call_config)
